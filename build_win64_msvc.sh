...
if [ ! -f build_protobuf/libprotobuf.dll ]; then
    ...
    PB_BUILD=$(pwd)
    cmake                                                                               \
        --toolchain $PB_BUILD/../../cmake_modules_windows/Windows.MSVC.toolchain.cmake  \
...
if [ ! -f build_gns/bin/libGameNetworkingSockets.dll ]; then
    ...
    GNS_BUILD=$(pwd)
    cmake                                                                                   \
        --toolchain $GNS_BUILD/../../cmake_modules_windows/Windows.MSVC.toolchain.cmake     \
        -DCMAKE_BUILD_TYPE=Release                                                          \
        -DProtobuf_USE_STATIC_LIBS=ON                                                       \
        -DProtobuf_LIBRARIES="-L$GNS_BUILD/../build_protobuf/lib"                           \
        -DProtobuf_LIBRARIES_PATH="$GNS_BUILD/../build_protobuf/lib"                        \
        -DProtobuf_INCLUDE_DIRS=$GNS_BUILD/../../3rdparty/protobuf/src                      \
        -DProtobuf_INCLUDE_DIR=$GNS_BUILD/../../3rdparty/protobuf/src                       \
        -DProtobuf_INCLUDE_DIR2=$GNS_BUILD/../../3rdparty/protobuf/third_party/abseil-cpp   \
        -DProtobuf_PROTOC_EXECUTABLE=$GNS_BUILD/../build_protoc/protoc                      \
        -D USE_CRYPTO="BCrypt"                                                              \
...
if [ ! -f build_physfs/libphysfs.a ]; then
    ...
    PHYSFS_BUILD=$(pwd)
    cmake                                                                                   \
        --toolchain $PHYSFS_BUILD/../../cmake_modules_windows/Windows.MSVC.toolchain.cmake  \
        -DCMAKE_BUILD_TYPE=Release                                                          \
...
#cmake .. --toolchain ../cmake_modules_windows/Windows.MSVC.toolchain.cmake -D USE_CRYPTO="BCrypt" -Dprotobuf_BUILD_TESTS=OFF
cmake                                                                   \
    ..                                                                  \
    --toolchain ../cmake_modules_windows/Windows.MSVC.toolchain.cmake   \
    &&                                                                  \
    make -j10 openjkdf2-64
cd ..
